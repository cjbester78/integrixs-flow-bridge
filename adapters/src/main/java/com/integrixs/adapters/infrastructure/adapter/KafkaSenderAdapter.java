...
import com.integrixs.adapters.config.KafkaSenderAdapterConfig;
...
/**
 * Kafka implementation of sender adapter (consumes messages from Kafka topics)
 * Follows middleware convention: Sender = receives data FROM external systems.
 */
@Slf4j
public class KafkaSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    
    private final KafkaSenderAdapterConfig config;
    private KafkaConsumer<String, String> consumer;
    private final AtomicBoolean polling = new AtomicBoolean(false);
    private Thread pollingThread;
    private final CountDownLatch shutdownLatch = new CountDownLatch(1);
    
    public KafkaSenderAdapter(KafkaSenderAdapterConfig config) {
        super();
        this.config = config;
    }
    ...
    protected AdapterOperationResult performInitialization() {
        try {
            log.info("Initializing Kafka sender adapter with bootstrap servers: {}", config.getBootstrapServers());
            
            validateConfiguration();
            
            log.info("Kafka sender adapter initialized for topics: {}", config.getTopics());
    ...
    public void startListening(DataReceivedCallback callback) {
        ...
        throw new UnsupportedOperationException("Use startPolling instead");
    }
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("Kafka Sender adapter - consumes messages from Kafka topics")
                .version("1.0.0")
                .supportsBatch(true)
                .supportsAsync(true)
                .build();

...
import com.integrixs.adapters.config.OdataSenderAdapterConfig;
 ...
 */
@Slf4j
public class OdataSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing OData sender adapter (inbound) with service URL: {}", config.getServiceUrl());
        ...
        log.info("OData sender adapter initialized successfully");
        return AdapterOperationResult.success("OData sender adapter initialized");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying OData sender adapter");
        processedEntities.clear();
        client = null;
        return AdapterOperationResult.success("OData sender adapter destroyed");
    }
    ...
    private AdapterOperationResult pollODataService() throws Exception {
        ...
        try {
            ...
            log.info("OData sender adapter retrieved {} entities", entities.size());

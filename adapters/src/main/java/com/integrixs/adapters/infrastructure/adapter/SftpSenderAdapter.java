 ...
 */
public class SftpSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing SFTP sender adapter (inbound) with server: {}:{}", 
        ...
        log.info("SFTP sender adapter initialized successfully");
    ...
    private AdapterOperationResult pollForFiles() throws Exception {
        ...
        log.info("SFTP sender adapter polled {} files from server", processedFiles.size());
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("SFTP Sender adapter - receives files from SFTP servers")
                .version("1.0.0")
                .supportsBatch(true)
                .supportsAsync(true)
                .build();

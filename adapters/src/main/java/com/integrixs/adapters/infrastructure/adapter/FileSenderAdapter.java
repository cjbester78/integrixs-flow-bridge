 ...
 */
@Slf4j
public class FileSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing File sender adapter (inbound) with directory: {}", config.getSourceDirectory());
        ...
        try {
            ...
            log.error("Failed to initialize File sender adapter", e);
            return AdapterOperationResult.failure("Initialization failed: " + e.getMessage());
        }
        
        log.info("File sender adapter initialized successfully");
        return AdapterOperationResult.success("File sender adapter initialized successfully");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying File sender adapter");
        processedFiles.clear();
        return AdapterOperationResult.success("File sender adapter shutdown successfully");
    }
    ...
    private AdapterOperationResult pollForFiles() throws Exception {
        ...
        log.info("File sender adapter polled {} files from directory", processedFilesList.size());

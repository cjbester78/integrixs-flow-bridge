...
import com.integrixs.adapters.config.JdbcReceiverAdapterConfig;
 ...
 */
@Slf4j
public class JdbcReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing JDBC receiver adapter (outbound) with URL: {}", maskSensitiveUrl(config.getJdbcUrl()));
        ...
        log.info("JDBC receiver adapter initialized successfully");
        return AdapterOperationResult.success("JDBC receiver adapter initialized successfully");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying JDBC receiver adapter");
        if (dataSource != null && !dataSource.isClosed()) {
            dataSource.close();
            dataSource = null;
        }
        return AdapterOperationResult.success("JDBC receiver adapter shutdown successfully");
    }

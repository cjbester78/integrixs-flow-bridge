 ...
 */
@Slf4j
public class HttpSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing HTTP sender adapter with endpoint: {}", 
        ...
        log.info("HTTP sender adapter initialized successfully");
        return AdapterOperationResult.success("HTTP sender adapter initialized successfully");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying HTTP sender adapter");
        // HttpClient doesn't need explicit cleanup
        httpClient = null;
        return AdapterOperationResult.success("HTTP sender adapter shutdown successfully");
    }
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("HTTP Sender adapter - receives data via HTTP endpoints")
                .version("1.0.0")
                .supportsBatch(true)
                .supportsAsync(true)
                .build();

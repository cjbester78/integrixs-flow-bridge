 ...
 */
@Slf4j
public class FtpReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing FTP receiver adapter (outbound) with server: {}:{}", 
        ...
        try {
            ...
            log.info("FTP receiver adapter initialized successfully");
            return AdapterOperationResult.success("Initialized successfully");
        } catch (Exception e) {
            log.error("Failed to initialize FTP receiver adapter", e);
    ...
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying FTP receiver adapter");
    ...
    private AdapterOperationResult uploadContentToFtp(String fileName, byte[] content, boolean isBatch, int itemCount) throws Exception {
        ...
        try {
            ...
            log.info("FTP receiver adapter uploaded {} bytes to file: {}", bytesUploaded, uploadPath);
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("FTP Receiver adapter - sends files to FTP servers")
                .version("1.0.0")
                .supportsBatch(true)
                .supportsAsync(true)
                .build();

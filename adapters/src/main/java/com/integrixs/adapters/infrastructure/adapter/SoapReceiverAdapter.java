 ...
 */
@Slf4j
public class SoapReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing SOAP receiver adapter (outbound) with endpoint: {}", config.getEffectiveEndpoint());
        ...
        log.info("SOAP receiver adapter initialized successfully");
        return AdapterOperationResult.success("SOAP receiver adapter initialized");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying SOAP receiver adapter");
        // Cleanup resources if needed
        return AdapterOperationResult.success("SOAP receiver adapter destroyed");
    }
    ...
    private AdapterOperationResult invokeSoapService(Object payload) throws Exception {
        ...
        try {
            ...
            log.info("SOAP receiver adapter successfully invoked service");
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("SOAP Receiver adapter - sends SOAP requests")
                .version("1.0.0")
                .supportsBatch(true)
                .supportsAsync(true)
                .build();

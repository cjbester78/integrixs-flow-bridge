...
import com.integrixs.adapters.config.IdocSenderAdapterConfig;
import java.util.*;
 ...
 */
@Slf4j
public class IdocSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing IDoc sender adapter (inbound) with program ID: {}", config.getProgramId());
        ...
        log.info("IDoc sender adapter initialized successfully");
        return AdapterOperationResult.success("IDoc sender adapter initialized successfully");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying IDoc sender adapter");
        if (serverStarted) {
            // In real implementation, would stop IDoc server
            serverStarted = false;
        }
        receivedIdocs.clear();
        return AdapterOperationResult.success("IDoc sender adapter shutdown successfully");
    }
    ...
    private AdapterOperationResult receiveIdoc(Object payload, Map<String, Object> headers) throws Exception {
        ...
        if (payload instanceof Map) {
            ...
            log.info("IDoc sender adapter received IDoc: {} of type: {}", 
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("IDoc Sender adapter implementation")
                .version("1.0.0")
                .supportsBatch(false)
                .supportsAsync(true)
                .build();

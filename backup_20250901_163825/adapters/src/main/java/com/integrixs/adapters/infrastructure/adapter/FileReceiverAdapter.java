 ...
 */
@Slf4j
public class FileReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing File receiver adapter (outbound) with directory: {}", config.getTargetDirectory());
        
        try {
            validateConfiguration();
            initializeDirectory();
        } catch (Exception e) {
            log.error("Failed to initialize File receiver adapter", e);
            return AdapterOperationResult.failure("Initialization failed: " + e.getMessage());
        }
        log.info("File receiver adapter initialized successfully");
        return AdapterOperationResult.success("File receiver adapter initialized successfully");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying File receiver adapter");
        ...
        batchBuffer.clear();
        return AdapterOperationResult.success("File receiver adapter shutdown successfully");
    }
    ...
    private AdapterOperationResult writeItemsToFile(Path targetFile, List<Object> items, boolean isBatch) throws Exception {
        ...
        try {
            ...
            log.info("File receiver adapter wrote {} bytes to file: {}", bytesWritten, targetFile);

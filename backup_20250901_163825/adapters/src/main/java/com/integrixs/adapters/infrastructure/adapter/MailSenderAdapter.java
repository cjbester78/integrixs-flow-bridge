...
import com.integrixs.adapters.config.MailSenderAdapterConfig;
import jakarta.mail.*;
 ...
 */
@Slf4j
public class MailSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing Mail sender adapter (inbound) with server: {}:{} using {}", 
        ...
        log.info("Mail sender adapter initialized successfully");
        return AdapterOperationResult.success("Mail sender adapter initialized");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying Mail sender adapter");
        disconnectFromMailServer();
        processedMessages.clear();
        return AdapterOperationResult.success("Mail sender adapter destroyed");
    }
    ...
    private AdapterOperationResult pollForEmails() throws Exception {
        ...
        log.info("Mail sender adapter polled {} emails from server", processedEmails.size());
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("Mail sender adapter - receives emails from mail servers")
                .version("1.0.0")
                .supportsBatch(false)
                .supportsAsync(true)
                .build();

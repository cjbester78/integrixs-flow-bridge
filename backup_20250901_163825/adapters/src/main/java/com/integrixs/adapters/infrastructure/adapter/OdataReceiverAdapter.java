 ...
 */
@Slf4j
public class OdataReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing OData receiver adapter (outbound) with service URL: {}", config.getServiceUrl());
        ...
        log.info("OData receiver adapter initialized successfully");
        return AdapterOperationResult.success("OData receiver adapter initialized");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying OData receiver adapter");
        client = null;
        return AdapterOperationResult.success("OData receiver adapter destroyed");
    }
    ...
    private AdapterOperationResult performODataOperation(Object payload) throws Exception {
        ...
        try {
            ...
            log.info("OData receiver adapter successfully performed operation");
    ...
    private AdapterOperationResult performMetadataValidationTest() {
        try {
            if (config.getNamespace() == null || config.getNamespace().isEmpty()) {
                return AdapterOperationResult.failure("Metadata Validation", "Namespace not configured", null);
            }
            return AdapterOperationResult.success("Metadata Validation", "Metadata configuration valid");
        } catch (Exception e) {
            return AdapterOperationResult.failure("Metadata Validation", "Metadata validation failed: " + e.getMessage(), e);
        }
    }
    ...
    private AdapterOperationResult performOperationConfigTest() {
        try {
            ...
            return AdapterOperationResult.failure("Operation Config", "Operation configuration test failed: " + e.getMessage(), e);
        }
    }

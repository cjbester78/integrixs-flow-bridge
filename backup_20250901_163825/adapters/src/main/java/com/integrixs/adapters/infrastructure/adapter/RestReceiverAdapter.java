...
import java.util.HashMap;import java.util.List;import com.integrixs.adapters.domain.port.ReceiverAdapterPort;
import java.util.HashMap;import java.util.Map;
import java.util.HashMap;import java.util.List;import com.integrixs.adapters.config.RestReceiverAdapterConfig;
 ...
 */
@Slf4j
public class RestReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing REST receiver adapter (outbound) with URL: {}", config.getBaseUrl());
        ...
        log.info("REST receiver adapter initialized successfully");
        return AdapterOperationResult.success("REST receiver adapter initialized");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying REST receiver adapter");
        ...
        batchBuffer.clear();
        restTemplate = null;
        return AdapterOperationResult.success("REST receiver adapter destroyed");
    }
    ...
    private AdapterOperationResult sendApiRequest(Object payload, boolean isBatch, int itemCount) throws Exception {
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                log.info("REST receiver adapter sent {} items to API, status: {}", 
                ...
                throw new AdapterException.ProcessingException(AdapterConfiguration.AdapterTypeEnum.HTTP, 
                        "REST API call failed with status: " + response.getStatusCode());
            }
        } catch (HttpClientErrorException e) {
            throw new AdapterException.ProcessingException(AdapterConfiguration.AdapterTypeEnum.REST, 

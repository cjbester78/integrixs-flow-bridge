...
import java.util.List;import com.integrixs.adapters.config.RestSenderAdapterConfig;
 ...
 */
@Slf4j
public class RestSenderAdapter extends AbstractAdapter implements SenderAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing REST sender adapter (inbound) with URL: {}", config.getBaseUrl());
        ...
        log.info("REST sender adapter initialized successfully");
        return AdapterOperationResult.success("REST sender adapter initialized");
    }
    
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying REST sender adapter");
        processedMessages.clear();
        restTemplate = null;
        return AdapterOperationResult.success("REST sender adapter destroyed");
    }
    ...
    private AdapterOperationResult pollFromRestApi() throws Exception {
        ...
        log.info("REST sender adapter polled {} items from API", processedData.size());

...
import com.integrixs.adapters.config.SftpReceiverAdapterConfig;
 ...
 */
public class SftpReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing SFTP receiver adapter (outbound) with server: {}:{}", 
    ...
    private AdapterOperationResult uploadContentToSftp(String fileName, byte[] content, boolean isBatch, int itemCount) throws Exception {
        ...
        try {
            ...
            log.info("SFTP receiver adapter uploaded {} bytes to file: {}", bytesUploaded, uploadPath);
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("SFTP Receiver adapter - sends files to SFTP servers")
                .version("1.0.0")
                .supportsBatch(true)
                .supportsAsync(true)
                .build();

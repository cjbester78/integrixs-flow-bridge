...
import com.integrixs.adapters.config.JmsReceiverAdapterConfig;
import jakarta.jms.*;
 ...
 */
public class JmsReceiverAdapter extends AbstractAdapter implements ReceiverAdapterPort {
    ...
    protected AdapterOperationResult performInitialization() {
        log.info("Initializing JMS receiver adapter (outbound) for destination: {}", config.getDestinationName());
        ...
        log.info("JMS receiver adapter initialized successfully");
        return AdapterOperationResult.success("JMS receiver adapter initialized");
    }
    @Override
    protected AdapterOperationResult performShutdown() {
        log.info("Destroying JMS receiver adapter");
        try {
            if (producer != null) {
                producer.close();
            }
        ...
        return AdapterOperationResult.success("JMS receiver adapter destroyed");
    }
    @Override
    ...
    private AdapterOperationResult sendJmsMessage(Object payload) throws Exception {
        try {
            ...
            log.info("JMS receiver adapter sent message with ID: {}", message.getJMSMessageID());
    ...
    public AdapterMetadata getMetadata() {
        return AdapterMetadata.builder()
                ...
                .description("JMS Receiver adapter - sends messages to JMS destinations")
                .version("1.0.0")
                .supportsBatch(true)
                .supportsAsync(true)
                .build();

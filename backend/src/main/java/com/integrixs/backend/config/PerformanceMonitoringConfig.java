...
public class PerformanceMonitoringConfig {
    ...
    public static class DataSourceMetrics {
        ...
        public DataSourceMetrics(DataSource dataSource, MeterRegistry registry) {
            ...
            // Register HikariCP metrics if available and not already set
            if(dataSource instanceof com.zaxxer.hikari.HikariDataSource) {
                var hikariDataSource = (com.zaxxer.hikari.HikariDataSource) dataSource;
                try {
                    // Only set MetricRegistry if it hasn't been set already
                    if(hikariDataSource.getMetricRegistry() == null) {
                        hikariDataSource.setMetricRegistry(registry);
                    }
                } catch(IllegalStateException e) {
                    // MetricRegistry already set, ignore
                    log.debug("MetricRegistry already set for HikariDataSource: {}", e.getMessage());
                }
            }
        }
    }
